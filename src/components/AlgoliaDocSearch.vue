...
const userOptions = {
  ...
  // Remove the strict filter if your index doesn't have language:java
  // searchParameters: {
...
function initialize() {
  const options = Object.assign({}, userOptions, {
    ...
    transformItems(items: any[]) {
      return items
        .filter(item => {
          // Exclude JavaScript results, allow everything else
          const isNotJavaScript =
            !(item.title && item.title.toLowerCase().includes('javascript')) &&
            !(item.url && item.url.toLowerCase().includes('javascript'));
          return isNotJavaScript;
        })
        .map((item) => {
